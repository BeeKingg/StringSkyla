...
ERROR_MESSAGE = "Oops! An exception occurred! \n\n**Error** : {} " \
            "\n\nTolong Laporan ke [Support](t.me/skylasupport) jika eror " \
...
async def generate_session(bot, msg, telethon=False):
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\nSupport by @skylaprojects".format("TELETHON" if telethon else "PYROGRAM", string_session)
    await client.send_message("me", text)
    await client.disconnect()
    await phone_code_msg.reply("Berhasil Megambil {} string session. \n\nSilahkan cek di Pesan Tersimpan/Saved Message! \n\nSupport @skylasupport".format("telethon" if telethon else "pyrogram"))
